...
type MasterSlave struct {
    ...
    storage           MasterStateStore // 主从状态存储
    scheduler         *StageScheduler  // 阶段调度器（Master时启动）
...
### 4.1 与 StageScheduler 协作

```mermaid
flowchart LR
    MS[MasterSlave] -->|"IsMaster"| Decision{是否Master?}
    Decision -->|"Yes"| Start[启动StageScheduler]
    Decision -->|"No"| Stop[停止StageScheduler]
    Start --> Scheduler[StageScheduler运行]
    Stop --> Idle[空闲等待]
```
...
sequenceDiagram
    ...
    participant Storage as MasterStateStore
...
```go
type MongoMasterStateStore struct {
    collection *mongo.Collection
}

func (s *MongoMasterStateStore) GetMasterState() (*MasterState, error) {
    // 查询 master_state 表的唯一记录
}

func (s *MongoMasterStateStore) SetMasterState(state *MasterState) error {
    // 使用 upsert，原子操作
    // 如果已存在其他 Master，返回错误
}

func (s *MongoMasterStateStore) UpdateMasterState(state *MasterState) error {
    // 更新心跳时间，条件：NodeId 匹配
}
```
...
> **说明**：MasterSlave 不使用 EventBus 进行事件通信，它通过**定时心跳机制**主动管理主从状态。这是设计决策：心跳检测需要主动周期探测，不适合事件驱动模式。

#### 接收事件

| 事件类型 | 事件来源 | 触发时机 | 处理流程 |
|---------|---------|---------|---------|
| 无 | - | - | MasterSlave 不订阅任何业务事件 |

#### 发出事件

| 事件类型 | 发布时机 | 目标模块 | 触发动作 |
|---------|---------|---------|---------|
| 无 | - | - | MasterSlave 不发布任何业务事件 |
...
flowchart TB
    subgraph 定时心跳流程
        Ticker[定时器<br/>每10秒触发] --> MR[manageRole]
        MR --> GS[storage.GetMasterState]
        GS -->|"不存在"| TSM1[trySwitchToMaster<br/>抢占成为Master]
        GS -->|"存在"| CHK[检查角色]
        CHK -->|"当前是Master"| UH[updateHeartbeat<br/>维持心跳]
        CHK -->|"当前是Slave"| CMH[checkMasterHeartbeat<br/>检查Master心跳]
        CMH -->|"心跳过期"| TSM2[trySwitchToMaster<br/>抢占成为新Master]
        CMH -->|"心跳正常"| WAIT[等待下一周期]
    end

    subgraph Master角色切换
        TSM1 --> STM[switchToMaster<br/>启动StageScheduler]
        UH -->|"被其他节点抢占"| STS[switchToSlave<br/>停止StageScheduler]
        TSM2 --> STM
    end

    note["MasterSlave 不使用 EventBus<br/>通过定时心跳主动管理状态"]
```

#### 与 StageScheduler 的协作方式

| 协作方式 | 说明 |
|---------|------|
| **直接调用** | MasterSlave 直接调用 scheduler.Start() / scheduler.Stop() |
| **非事件驱动** | 不通过 EventBus 发布"切换Master"等事件 |
| **原因** | 角色切换是内部管理逻辑，不需要通知其他模块 |
...
- [阶段调度器设计](阶段调度器设计.md) - StageScheduler 与 MasterSlave 协作
