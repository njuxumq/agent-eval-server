...
| StageSplitter     | 批量创建stages表记录、发布StageSplitCompleteEvent              | 子任务拆分             | EventBus, TaskStore, TaskConfigStore, StageStore                 |
...
| QuotaAllocator    | 批量配额门控、发布AllocationEvent                            | 分发执行               | EventBus, QuotaManager, SubTaskStore               |
...
flowchart TB
    ...
    StageSplitter --> TaskStore
    StageSplitter --> TaskConfigStore
