...
调度服务负责任务生命周期管理、DAG调度、子任务分发。采用事件驱动架构，各模块通过 EventBus 进行异步通信。

**核心模块**：TaskManager、TaskSplitter、DAGBuilder、DAGScheduler、Dispatcher、ExecutorManager、EventBus
...
- [任务拆分器设计](details/任务拆分器设计.md) - TaskSplitter 实现细节
- [DAG构建器设计](details/DAG构建器设计.md) - DAGBuilder 实现细节
- [事件总线设计](details/事件总线设计.md) - EventBus 实现细节
- [DAG调度器设计](details/DAG调度器设计.md) - DAGScheduler 实现细节
- [子任务处理器设计](details/子任务处理器设计.md) - SubTaskHandler 实现细节
